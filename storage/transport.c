 ...
 *
 * Modifited by Raz Zohar in 24/04/2016 , based on the Matthew Dharm's version 
 *
...
//-------------------------------------------------------------------------------------------
//BadBlock USB-storage HUB via socket


/**
This function create a simple AF_UNIX socket and connect by the path name
@param path_name - string contain the path of the socket
@retval SOCKET_ERROR if something wrong otherwise return 0.

*/
int socket_init(char * path_name)
{

	int retval;
	struct sockaddr_un addr;
	  
	printk(KERN_INFO "Create socket...\n");
	...
	strcpy(addr.sun_path, path_name);
	  ...
	  //sizeof(addr) - 1 for the null termination
	retval = sock->ops->connect(sock, (struct sockaddr *)&addr, sizeof(addr) - 1, 0);
	printk(KERN_INFO "Connect to the socket...\n");

	return(retval);
}
EXPORT_SYMBOL_GPL(socket_init);
...
void socket_close(void)
{
    // release socket
    sock_release(sock);
    printk(KERN_INFO "Close the socket...\n");
}
EXPORT_SYMBOL_GPL(socket_close);



/**
	This function read the data in the socket and store it into str
	@param str - string to store in the data that read from socket
	@retval - bytes that read from the socket
...
int socket_read(char * str)
{
	...
	//Recv the message
	retval = sock_recvmsg(sock, &msg, strlen(str), 0);

	set_fs(oldfs);

	return(retval);
}
EXPORT_SYMBOL_GPL(socket_read);


/**
	This function write the data to the socket and store it into str
	@param str - string to store that send to socket
...
int socket_write(char * str)
{
    ...
    retval = sock_sendmsg(sock, &msg, strlen(str));
...
EXPORT_SYMBOL_GPL(socket_write);

/**
This function send the command to the ep that has been read from the socket
@param struct scsi_cmnd *srb - struct that shows the scsi cmd
	   struct us_data *us - struct of the data
@Rretval - the return value of the usb_stor_bulk_transfer_buf function if read from socket sucess
otherwise return !USB_STOR_XFER_GOOD
*/
int send_command(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	memset(us->iobuf, 0, sizeof(us->iobuf));
	bytes_read = socket_read(us->iobuf);

	bcb = (struct bulk_cb_wrap *) us->iobuf;
	
	if (bytes_read)
	{
		printk("Read from socket sucess...\n");
...
/**
This function Get context switch And read data from DOK & send to the socket
@param struct scsi_cmnd *srb - struct that shows the scsi cmd
	   unsigned int *cswlen - pointer to length of the cs comaand
@Rretval - the return value of the usb_stor_bulk_transfer_buf function if write to socket sucess
otherwise return !USB_STOR_XFER_GOOD

*/
int get_context_switch(struct us_data *us, unsigned int *cswlen)
{
	unsigned int bytes_written = 0;
	struct bulk_cs_wrap *bcs = (struct bulk_cs_wrap *) us->iobuf;
	int result = usb_stor_bulk_transfer_buf(us, us->recv_bulk_pipe,
				bcs, US_BULK_CS_WRAP_LEN, cswlen);

	bytes_written = socket_write(us->iobuf);

	return(result);
}
EXPORT_SYMBOL_GPL(get_context_switch);


/**
This function read data from socket and send to the device or recv data from socket and write to the device
@param struct scsi_cmnd *srb - struct that shows the scsi cmd
	   struct us_data *us - struct of the data
@Rretval - the return value of the usb_stor_bulk_srb function

*/
int socket_srb(struct scsi_cmnd *srb, struct us_data *us)
{
	int result;
	unsigned int bytes_written = 0;
	unsigned bytes_read = 0;
	...
	if(pipe == us->send_bulk_pipe)
	{
		//READ DATA FROM SOCKET and send to the pipe
		memset(us->iobuf, 0, sizeof(us->iobuf));
		bytes_read = socket_read(us->iobuf);
	}	
	...
	//RECV from pipe and send to socket
	if(pipe == us->recv_bulk_pipe)
	{
		bytes_written = socket_write(us->iobuf);
	}
...
int usb_stor_Bulk_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	//Send the commant that recieve from socket
	result = send_command(srb, us);
	...
	result = get_context_switch(us, &cswlen);
	...
	if (result == USB_STOR_XFER_STALLED)
	{
		...
		result = get_context_switch(us, NULL);
